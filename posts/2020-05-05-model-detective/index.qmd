...
dat <- d %>%
  ...
  filter(date >= "2020-02-26" & date < "2020-05-05") %>%   
...
```{r}
p <- fitted(lm(log(death + 0.5) ~ poly(as.numeric(date), 3), data = dat))
```
...
p <- predict(lm(log(death + 0.5) ~ poly(as.numeric(date), 3), data = dat), newdata = new_data)
...
p2 <- predict(lm(log(death + 0.5) ~ poly(as.numeric(date), 3), data = dat), newdata = new_data2)
