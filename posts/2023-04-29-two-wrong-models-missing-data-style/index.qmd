...
```
Oops! As expected, we have some bias here. Specifically, we are off by 0.16. Now let's do some imputation. I am going to use the `mice` package. I am going to fit the "wrong" imputation model, only including `c_wrong`. 
...
Oh no, very bad! The wrong imputation model is worse than complete case! By a lot! This estimate is off by 0.58. Does conditioning on `c_wrong` help us at all?
